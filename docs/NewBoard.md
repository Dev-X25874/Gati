...
[    7.614143] rockchip_clk_register_frac_branch: could not find dclk_vop0_frac as parent of dclk_vop0, rate changes may not work
[    7.621322] rockchip_clk_register_frac_branch: could not find dclk_vop1_frac as parent of dclk_vop1, rate changes may not work
[   11.580546] rockchip-vop ff8f0000.vop: Adding to iommu group 3
[   11.580877] rockchip-vop ff900000.vop: Adding to iommu group 4
[   11.922731] rockchip-drm display-subsystem: bound ff8f0000.vop (ops 0xffffffc0095b80c0)
[   11.922957] rockchip-drm display-subsystem: bound ff900000.vop (ops 0xffffffc0095b80c0)
[   11.943093] rockchip-vop ff8f0000.vop: [drm:vop_crtc_atomic_enable] Update mode to 320x240p360, type: 14
```
Check if the resolution in the last line is **320x240p360**; if it is not, get the bridge's firmware reflashed. A newer version of Rah may have modified the resolution; check with the Gati team.
That is resolution of MIPI.

#### 4. IP conflicts
Before we setup SSH, you need to access the board using UART.
...
#### 5. Dependancies for Gati
Install [pyrah](https://github.com/vicharak-in/pyrah) and [Bitman](https://github.com/vicharak-in/Bitman).

#### 5. Run Gati
Check with the Gati team about the stable version of  **Gati** (FPGA) and **gaticc** (CPU).  
Take bitstream from the [**Gati Server**](http://galactos.local:8471/).
