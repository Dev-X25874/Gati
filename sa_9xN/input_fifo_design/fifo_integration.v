module fifo_integration #(
    ...
    output o_tx_debug_data_1,
    output o_tx_debug_data_2,
    output [(COL * W_ADDR) : 0] o_north_occupants,
    output [4:0] trigger_cnt
);
...
.CLKS_PER_BIT(37)
   ...
   .i_Rst_L(1'b1),
   .i_Clock(i_clk),
   .i_TX_DV(column_fifo_read_enable),
   .i_TX_Byte(fifo_1_o_data), 
   .o_TX_Active(),
   .o_TX_Serial(o_tx_debug_data_1),
   .o_TX_Done()
   );
...
north_array_dbg (
    .clk(i_clk), 
    .rst(1'b1),
    .fifo_we(weight_fifo_dv),
	.data_in(fifo_1_o_data),
	.tx(o_tx_debug_data_1),
    .full()
);
*/
//fifo#(
...
.CLKS_PER_BIT(37)
   ...
   .i_Rst_L(1'b1),
   .i_Clock(i_clk),
   .i_TX_DV(row_fifo_read_enable),
   .i_TX_Byte(fifo_2_o_data), 
   .o_TX_Active(),
   .o_TX_Serial(o_tx_debug_data_2),
   .o_TX_Done()
   );
...
udg_8 data_fifo_dbg(
    .clk(i_clk), 
    .rst(1'b1),
    .fifo_we(row_fifo_read_enable),
	.data_in(fifo_2_o_data),
	.tx(o_tx_debug_data_2),
    .full()
);

//Uart transmitter for debugging on logic analyzer
uart_tx#(
    .CLKS_PER_BIT(10)
) fifo_data_tx_2 (
    .i_Rst_L(1'b1),
...
udg uart_debug(
    .clk(i_clk), 
    .rst(1'b1),
    .fifo_we(i_north_rden),
	//input [63:0]data_in,
	.data_in(o_north_data),
    .tx(o_tx_debug_data_1),
    .full()
);*/
