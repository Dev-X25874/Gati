//Includes fifo, array of fifo and controllers to store input weights and data
module fifo_integration #(
    ...
    output [(COL * W_DATA) -1 : 0] o_north_data,
    output [COL-1:0] i_north_array_dv,
    output [(ROW * (W_DATA+1))-1 : 0] out_west_data,
...
//controlls enable signal of weight_fifo and sr_column
col_buf_controller#(
    .COL(COL),
    .ROW(ROW),
    .W_ADDR(W_ADDR)
...
//controlls enable signal of data_fifo and sr_row
row_buf_controller#(
    .COL(COL),
    .ROW(ROW),
    .W_ADDR(W_ADDR)
...
wire [W_DATA-1 : 0] fifo_1_col_o_data;
wire empty_2; 
wire full_2; 
...
//Append data valid bit with the data of each row
in_sa_row_data#(
    .ROW(ROW),
    .W_DATA(W_DATA)
) i_sa_row (
