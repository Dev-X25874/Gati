...
synchronous_fifo  #(.DEPTH(DEPTH),.DATA_WIDTH(DATA_WIDTH),.STATUS_DRAM_LIM(STATUS_DRAM_LIM))instant(.clk(clkin),.rst_n(1'b1),.w_en(instruct_valid),.r_en(read_req_inst),.data_in(instruct_mem),.data_out(o_instruction),.data_out_valid(o_instruction_valid),.full(),.empty(),.ten_trigg(o_status_dram),.not_empty(o_status_inst));
endmodule