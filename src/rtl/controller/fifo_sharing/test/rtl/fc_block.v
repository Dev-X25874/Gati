...
module fc_block#(
    ...
    // input [(N_SA * W_DATA)-1 : 0] i_north_data,  //Reads data from one north fifo array at a time
    input [(N_SA * (COL * (W_DATA+1)))-1 : 0] i_north_data, //Reads data from all the fifo in array simultaneously
    // input [(N_SA * W_DATA)-1 : 0] i_west_data,
    input [(N_SA * (ROW * (W_DATA+1)))-1  :0] i_west_data,
    input [N_SA-1 : 0] i_west_dv,
...
generate
    ...
    fc_sa_engine#(
        ...
        .i_sel_1(i_sel_1),
        .i_north_empty(i_north_empty[((COL * (N_SA - i))-1) -: COL]),
        .i_north_occ(i_north_occ[((COL * (W_ADDR + 1)) * (N_SA - i))-1 -: (COL * (W_ADDR + 1))]),
        .o_north_rden(o_north_rden[(COL * (N_SA - i))-1 -: COL]),
        .i_img_dim(i_acc_image_dim),
        // .i_north_data(i_north_data[(W_DATA * (N_SA - i))-1 -: W_DATA]),
        .i_north_data(i_north_data[((COL * (W_DATA + 1)) * (N_SA - i)) -: (COL * W_DATA)]),
        ...
        .i_west_dv(i_west_dv[(ROW * (N_SA - i)) -: ROW]),
