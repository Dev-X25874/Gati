        ...
        case(state)
            ...
            if(c_ctr==i_channel_itr-1) begin
                ...
                if(counter==3) begin
                    ...
                    state        <=  3'd3;  
                    count2 <= count2 + (NUMBER_OP*(r_burst_len2+1)); 
                end
                else begin
                ...
                if(counter==3) begin
                    ...
                    state        <=  3'd3;  
                    count1 <= count1 + (NUMBER_ACC*(r_burst_len1+1)); 
                end
                else begin
                    for(i=0;i<3;i=i+1) begin
                        if(counter==i) begin
                ...
                if(c_ctr < i_channel_itr-1) begin
                    if(count1==i_imag_dim) begin
                        ...
                        if(data_last) begin 
                            state <= 4;
                            count1 <= 0;
                            r_acc_start_add<=r_acc_start_add+((r_burst_len1+1)<<$clog2(AXI_DATA_BYTES));
                        end
                        else begin
                            state<=state;
                    ...
                    if(count2==i_imag_dim_2) begin
                        ...
                        if(data_last) begin 
                            state <= 4;
                            r_layer_start_add<=r_layer_start_add+((r_burst_len2+1)<<$clog2(AXI_DATA_BYTES));
                            count2 <= 0;
                        end
                        else begin
                            state<=state;
