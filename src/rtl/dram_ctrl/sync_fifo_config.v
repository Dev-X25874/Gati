module synchronous_fifo #(parameter DEPTH=512, DATA_WIDTH=256) (
    input clk, rst_n,
    input w_en, r_en,
    ...
    output full, empty,
    output [($clog2(DEPTH)+1):0]occupants,
    output ten_trigg,
    output not_empty
  );

  reg [($clog2(DEPTH)-1):0] w_ptr=1'b0;
  reg [($clog2(DEPTH)-1):0] r_ptr=1'b0;
  reg [DATA_WIDTH-1:0] fifo[DEPTH:0];
  reg [($clog2(DEPTH)+1):0]occupants_reg=1'b0;
