module top_im2col #(
    ...
    index_coordinate #(.UPPER_BOUND(UPPER_BOUND), .LOWER_BOUND(LOWER_BOUND), .DATA_WIDTH(DATA_WIDTH)) index_dut(
        .clk(clk_in),
        .rstn(rstn),
        .i_data(i_data),
    ...
    bound_generation #(.UPPER_BOUND(UPPER_BOUND), .LOWER_BOUND(LOWER_BOUND), .DATA_WIDTH(DATA_WIDTH), .KERNEL_SIZE(KERNEL_SIZE)) bound_dut(
        .clk(clk_in),
        .rstn(rstn),
        .i_valid(valid),
    ...
    delay_reg #(.DATA_WIDTH(DATA_WIDTH), .KERNEL_SIZE(KERNEL_SIZE)) delay_dut(
    .clk(clk_in),
    .rst(rstn),
    ...
    stride_block #(.DATA_WIDTH(DATA_WIDTH), .KERNEL_SIZE(KERNEL_SIZE), .UPPER_BOUND(UPPER_BOUND)) stride_dut(
    .clk(clk_in),
    .rst(rstn),
    .stride(stride),
    .curr_col(col),
