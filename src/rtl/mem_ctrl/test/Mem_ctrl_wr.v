        ...
        case (state)
            ...
            MEM_DATA : begin 
                // if (count_blen == BLEN_w +1) begin 
                    if ((count_blen > (BLEN_w)) & DataWrEnd) begin 
                        wvalid_w <= 0 ;
                       // wlast_w  <= 0; 
                       // WriteEn <= 0;
                        count_blen <= 0 ;
...
assign DataWrEnd = (DataWrLast & wvalid_w & wready_w);
assign wlast_w = DataWrEnd;
...
always  @( posedge clk)  begin 
    ...
    else if (wvalid_w & wready_w &&(count_blen == BLEN_w)) begin
        DataWrLast <= 1;
    end

    else if (DataWrEnd) begin
        DataWrLast <= 0;
    end
