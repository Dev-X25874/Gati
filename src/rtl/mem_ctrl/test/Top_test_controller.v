...
///// generating the data and passing to the DDR (only for testing) //////
Mem_ctrl_wr # (
.AXI_DATA_WIDTH (AXI_DATA_WIDTH)
)
Mem_ctrl_wr_1(
...
Mem_ctrl_wr_2 (
 ...
 /// mux is selecting the data according to select line from write ID manager which write port can give the data///
 MUX_WR_SEL 
 Mux_inst(
    .clk(clk),
    .rst (Axi0Rst_N), 
...
///// ID manager generate the select line according to id which is come from DDR ////
WR_ID_Manager #(
...
ID_MANAGER_inst(
    ...
    .wlast (wlast) ,           // (I) from the external port which gives the write data here from MUX-DDR-this port 
