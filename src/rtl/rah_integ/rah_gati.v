...
///globalutput [(OP_FIFO*DATA_WIDTH_OB)-1:0] op_dram_fifo
	local  parameter INST_QUEUE_DEPTH = 512;
	local  parameter DRAM_IMG_FIFO_DEPTH = 512;
	local  parameter IM2COL_FIFO_DEPTH = 1024;
	local  parameter WEIGHT_FIFO_DEPTH = 512;
	local  parameter PSUM_FIFO_DEPTH = 8192;
	local  parameter ACC_FIFO_DEPTH = 512;
	local  parameter BIAS_FIFO_DEPTH = 512; //For both conv and FC
	local  parameter OP_WRITE_FIFO_DEPTH = 2048;
     //Default burst lenghts for various memory request controllers
	local  parameter CONFIG_REQ_BLEN = 7;
	local  parameter IMG_REQ_BLEN = 15;
	local  parameter WEIGHT_REQ_BLEN = 15;
   local  parameter ACC_REQ_BLEN = 15;
   local  parameter OP_WRITE_REQ_ACC_BLEN = 31; //burst length for writng accumulants (32-bit) into the DRAM
   local  parameter OP_WRITE_REQ_QUA_BLEN = 15; //burst length for writng quantized wire (8-bit) into the DRAM

   local  parameter CONFIG_FIFO_OCCUPANCY = 10;
   local  parameter LAYERCNT_WIDTH = 12;
   local  //parameters related to DRAM controller
   local  parameter NUM_PORTS = 4; //Number of read and write requestors
   local  /*
   ...
   local  parameter AXI_DATA_BYTES = 32;  // Axi Data width = 256 bit
   local  parameter AXI_ADDR_W = 32;          // Axi Address width
   local  parameter BURST_LENGTH_WIDTH =8;
   local 
   local  parameter NUM_INSTRUCTIONS = 4;
   local  parameter INST_W = 256;

   local  parameter OPCODE_WIDTH = 4;
   local  parameter N_SA = NSA_DSP + NSA_LUT;
   local  parameter DATA_WIDTH = 8;
   local  parameter COL_SA = 4;
   local  parameter COL_FC = 32;
   local  parameter W_QUANT_SHIFT = 5;
   local  parameter W_QUANT_SCALE = 16;
   local  parameter ROW = 9;
   local  parameter W_PSUM = 20;
   local  parameter MOD1 = 2;
   local  parameter MOD2 = 8;
   local  parameter DATA_WIDTH_OB = 32;
   local  parameter IMAGE_DIM = 224;
   local  parameter SHFT_REG_X = 4; // Number of shift register blocks
   local  parameter BIAS_FIFO = 8; // Number of bias FIFOs
   local  parameter OP_FIFO = 8;  // Number of wire write FIFOs
   local  parameter NSA_DSP = 4; 
   local  parameter N_FC_MUX = 4;
   local  parameter NO_PORT_FC = 8;
   local  parameter RELU_CLIP_WIDTH = 8;
   local  parameter NSA_LUT = 0;
   local  parameter BIAS_FIFO_FC = 32; // Number of FC bias FIFOs
   local  parameter NO_PORT_VA = 2;
   local  parameter NO_PORT_BAC = 2;
   local  parameter NO_PORT_BAFC = 8;
   local  parameter POP_THRESHOLD = 5;
   local  parameter I_SIZE_WIDTH=20; // bit width of wire image dimension
   local  parameter ACC_DW = 32;
   local  parameter N_BANK = 4;
   local  parameter N_BRAM = 8;
   local  parameter W_FC_RW_COUNTER = 10; //width of fc r/w address counter
   local  parameter FC_BRAM_DEPTH = 1024;
   local  parameter W_KERNEL_CNT = 16;
   local  parameter W_IMAG_DIM = 20;
	local parameter ACC_DATA_REORDER = 1;
	wire i_clk,
    wire s_clk,
    wire i_rst,
