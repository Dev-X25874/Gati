...
generate
if(NSA_DSP == 0)begin
        test_sa_engine_dsp dsp_engine_inst (
            .i_clk(),
            .s_clk(),
            .i_rstn(),
            .i_trigger_1(),
        ...
        for(i = 0; i < NSA_DSP; i = i +1)begin : DSP_SA
            test_sa_engine_dsp#(
                .W_DATA(W_DATA),
                .COL(COL),
                .ROW(ROW),
...
generate
    if(NSA_LUT == 0)begin
            test_sa_engine_lut lut_engine_inst (
                .i_clk(),
                .s_clk(),
                .i_rstn(),
                .i_trigger_1(),
            ...
            for(j = 0; j < NSA_LUT; j = j + 1)begin : LUT_SA
                test_sa_engine_lut#(
                    .W_DATA(W_DATA),
                    .COL(COL),
                    .ROW(ROW),
                    ...
                    .i_rstn(i_rstn),
                    .i_trigger_1(i_trigger_1),
                    .i_weight_fifo_array_data(i_weight_fifo_array_data[(W_DATA * ((N_SA-NSA_DSP) - j))-1 -: W_DATA]),
                    .i_weight_fifo_array_write_en(i_weight_fifo_array_write_en[(COL * ((N_SA-NSA_DSP) - j))-1 -: COL]),
                    .i_image_fifo_array_data(i_image_fifo_array_data[(W_DATA * ((N_SA-NSA_DSP) - j))-1 -: W_DATA]),
