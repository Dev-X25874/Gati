...
assign mc_img_last   = ((opcode == `OP_POOL) | (opcode == `OP_RESIZE)) ? mc_img_last_pool_resize   : mc_img_last_conv; 
  ...
  */
  ...
  (* syn_use_dsp = "no" *) wire [I_ACC_SIZE_WIDTH-1:0] op_img_size; // MANTRA this is used in top_pool
  ...
  // MANTRA keep top_tailblock here
  top_tailblock#(
      .OPCODE_WIDTH(OPCODE_WIDTH),
      .N_SA(N_SA),
      .ACC_DW(ACC_DW),
      ...
      .NO_PORT_VA(NO_PORT_VA),
      .I_ACC_SIZE_WIDTH(I_ACC_SIZE_WIDTH),
      .W_CONV_OP_IMAGE_DIM(OutputBlock_OW_WIDTH), // MANTRA same input goes into 2 paramters
      .OutputBlock_OH_WIDTH(OutputBlock_OH_WIDTH),
      .OutputBlock_OW_WIDTH(OutputBlock_OW_WIDTH),
      .BIAS_FIFO_DEPTH(BIAS_FIFO_DEPTH),
      .BIAS_FIFO(BIAS_FIFO),
      ...
      .data_tail_blk_in(data_tail_blk_in), // MANTRA define these 2 
      .data_tail_blk_vaild(data_tail_blk_vaild), // MANTRA define these 2
      .iteration_Done(iter_done),
      .vector_add_values(vector_add_values),
      .op_full(op_full),
      .sa_stall(sa_stall),
...
*/
  ...
  `else
  assign datasize_fpga2cpu = (OutputBlock_FlatController)? img_dim_Op : img_dim_Op*kernel_iteration*N_SA*OB_OpWidth;
  `endif //FC
